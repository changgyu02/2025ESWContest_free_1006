...
public:
  PlannerNode() : Node("planner_node") {
    ...
    robot_radius_m_         = this->declare_parameter<double>("robot_radius_m", 0.13);
    max_snap_radius_m_      = this->declare_parameter<double>("max_snap_radius_m", 0.3);   
    ...
    // DownSampling param
    downsample_interval_m_  = this->declare_parameter<double>("downsample_interval_m", 0.06); // 기본 간격
...
private:
  ...
  std::vector<cv::Point> a_star(cv::Point start, cv::Point goal) {
    ...
    std::vector<cv::Point> dirs = {{1,0},{0,1},{-1,0},{0,-1}};
    ...
    while (!open.empty()) {
      ...
      for (auto d : dirs) {
        ...
        if (!g.count(nxt) || cost < g[nxt]) {
          g[nxt] = cost;
          double pr = cost + cv::norm(nxt - goal);
          open.push({pr, nxt});
          parent[nxt] = cur;
        }
      }
